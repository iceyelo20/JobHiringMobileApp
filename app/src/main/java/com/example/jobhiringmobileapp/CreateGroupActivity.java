...
public class CreateGroupActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        GroupTitle = (EditText) findViewById(R.id.group_title);
        GroupDescription = (EditText) findViewById(R.id.group_description);
        CreateGroupButton = (FloatingActionButton) findViewById(R.id.create_group_button);
    ...
    private void CreatingAGroup() {
        ...
        String groupTitle = GroupTitle.getText().toString().trim();
        String groupDescription = GroupDescription.getText().toString().trim();

        if(TextUtils.isEmpty(groupTitle)){
            Toast.makeText(this, "Please write a group name.", Toast.LENGTH_SHORT).show();
            return;
        }
        ...
        if(image_uri == null){
            ...
            storageReference.putFile(image_uri).addOnSuccessListener(new OnSuccessListener<UploadTask.TaskSnapshot>() {
                ...
                public void onFailure(@NonNull Exception e) {
                    ...
                    Toast.makeText(CreateGroupActivity.this, "Error Occurred: "+e.getMessage(), Toast.LENGTH_SHORT).show();
                }
            });
        }
    }
    ...
    private void createGroup(String g_timestamp, String groupTitle, String groupDescription, String groupIcon) {
        ...
        GroupRef.child(g_timestamp).setValue(hashMap).addOnSuccessListener(new OnSuccessListener<Void>() {
                @Override
                public void onSuccess(Void aVoid) {
                    ...
                    DatabaseReference GroupRef1 = FirebaseDatabase.getInstance("https://job-hiring-mobile-app-e1345-default-rtdb.asia-southeast1.firebasedatabase.app/").getReference().child("Groups");
                    GroupRef1.child(g_timestamp).child("Participants").child(currentUserID).setValue(hashMap1)
                            .addOnSuccessListener(new OnSuccessListener<Void>() {
                                @Override
                                public void onSuccess(Void aVoid) {
                                    progressDialog.dismiss();
                                    Intent groupIntent = new Intent(CreateGroupActivity.this, GroupChatActivity.class);
                                    groupIntent.putExtra("groupId", g_timestamp);
